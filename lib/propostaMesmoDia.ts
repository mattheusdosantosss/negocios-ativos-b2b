...
export type PMDCloser = {
  ownerId: string;
  nome: string;
  semComp: number; semElig: number;
  comComp: number; comElig: number;
...
export async function fetchPropostaMesmoDia(
  ...
  // 3) Reuniões de cada negócio (define com/sem reunião) + dias das reuniões.
  ...
  for (let i = 0; i < allM.length; i += 100) {
    ...
    for (const m of mr.results ?? []) {
      const dk = dayKey(toMs(m.properties.hs_meeting_start_time));
  ...
  for (const d of deals) {
    ...
    const meetIds = assoc.get(d.id) ?? [];
    const meetDays = new Set(meetIds.map((m) => mDay.get(m)).filter(Boolean) as string[]);
    const criadoMs = toMs(d.properties.createdate);
    const dl = (ok: boolean): PMDDeal => ({ dealname: d.properties.dealname || `Negócio ${d.id}`, url: dealUrl(d.id), ok, criadoMs, propMs });
    const c = get(oid);
    if (meetDays.size > 0) {
      const ok = !!propDay && meetDays.has(propDay);
      c.comElig += 1; if (ok) c.comComp += 1; c.dealsCom.push(dl(ok));
    } else {
      const ok = !!propDay && !!qualDay && propDay === qualDay;
      c.semElig += 1; if (ok) c.semComp += 1; c.dealsSem.push(dl(ok));
    }
  }
